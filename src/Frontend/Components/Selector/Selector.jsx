...
const Selector = ({ servers, setCreateServerPopup }) => {
  ...
  const handleServerClick = (server) => {
    ...
    const firstPublicChannel = server.channels.find(
      (channel) => channel.type === "public"
    );
    const firstServerChannel = server.channels.find(
  ...
  return (
    <div className="w-full min-h-[4.75rem] z-10 flex relative items-center overflow-x-auto  unselectable px-3 bg-[#2A2821] space-x-2">
      <div
        key="inbox"
        onClick={() => handleHoverForNow("Inbox will be coming soon!")}
        onMouseEnter={() => setHoveredServer("inbox")}
        onMouseLeave={() => setHoveredServer(null)}
        className="flex flex-col items-center"
      >
        <div
          className={`h-[3.6rem] w-[3.6rem] flex items-center justify-center cursor-pointer transition-all duration-300 ease-in-out mb-1`}
          style={{
            borderRadius:
              hoveredServer === "inbox" || selectedServer === "inbox"
                ? "1.15rem"
                : "50%",
            backgroundColor: selectedServer === "inbox" ? "#3F310E" : "#383631",
          }}
        >
      ...
      <div className="w-[0.6rem] flex justify-center">
        <div className="h-7 w-[1px] bg-[#B7B7B7]"></div>
      </div>
      {servers.map((server) => (
        <div
          key={server._id}
        ...
        >
          <div
            ...
            style={{
              ...
              borderRadius:
                hoveredServer === server._id || selectedServer === server._id
                  ? "1.15rem"
                  : "50%",
            }}
          >
      ...
      >
        <div
          ...
          style={{
            borderRadius:
              hoveredServer === "add" || selectedServer === "add"
                ? "1.15rem"
                : "50%",
            backgroundColor: selectedServer === "add" ? "#3F310E" : "#383631",
          }}
        >
      ...
      <div
        key="discover"
        onClick={() => handleHoverForNow("Will come soon! Send invites to your friends for now.")}
      ...
      >
        <div
          ...
          style={{
            borderRadius:
              hoveredServer === "discover" || selectedServer === "discover"
                ? "1.15rem"
                : "50%",
            backgroundColor:
              selectedServer === "discover" ? "#3F310E" : "#383631",
          }}
        >
